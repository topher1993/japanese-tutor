/**
 * Phase 25 / P2-1 — Hardcoded flashcard due dates → learner-local today.
 *
 ...
 *   2. flashcardService.ts uses the shared localDateKey() for nextReviewDate.
...
describe('Phase 25 / P2-1 — Hardcoded flashcard due dates removed', () => {
  ...
  it('flashcardService.ts uses the shared learner-local date helper', () => {
    const src = readFileSync(FLASH, 'utf8');
    expect(src).toContain("import { addLocalDateDays, localDateKey } from '../utils/localDate'");
  ...
  it('candidateFlashcardAdapter.ts uses the computed today for nextReviewDate', () => {
    const src = readFileSync(CAND, 'utf8');
    expect(src).toContain("import { localDateKey } from '../utils/localDate'");
    expect(src).toMatch(/nextReviewDate:\s*today\b/);
  });
});
